...
- 2026-05-02 stage 1.4 MVP scaffold landed (commit fad7304): kernels/aie2p/asym3_dequant_layer/ with C++ kernel + aie.mlir + build.sh; built clean on hipx (PDI 5088 B, insts 420 B). N_ITERS=32 hardcoded for the MVP; production-shape (full 27B layer = 32768 iters) + multi-core fan-out + shape variants follow in next iters. Embedded surface in crates/hipx/src/kernels.rs (ASYM3_DEQUANT_LAYER_*; KERNEL_ID=0x902; PACKED_BYTES=3072, CNORM_BYTES=128, OUT_BYTES=16384). Same compute path as asym3_dequant_256 per iteration; batching is in the host runtime DMA strides + core-side scf.for. Next: dispatch validation smoke test + scale N_ITERS toward production.
- 2026-05-02 stage 1.4 codex stop-time review caught kernel-id mismatch (commit 73c8fd6): aiecc emits dpu_kernel_id=0x901 by default for every single-kernel xclbin; I had set ASYM3_DEQUANT_LAYER_KERNEL_ID=0x902 in kernels.rs which did not match the generated PDI. Confirmed via build/aie.mlir.prj/main_kernels.json and via the convention used by every other kernel constant in kernels.rs. Comment added so future kernels do not repeat the divergence.
- 2026-05-02 stage 1.4 layer verifier landed + PASSES on hipx (commit 08badd6): verify_asym3_dequant_layer.rs generates N_ITERS=32 random (packed, cnorm) pairs per seed, dispatches the layer kernel ONCE, compares every iteration's 256 outputs against the AIE-2P-shape CPU reference. 100 seeds = 819200 element comparisons. Result: max ULP 2, mean signed +0.0155, determinism PASS across two-dispatch repeats per seed. The iteration mechanism (DMA strides + scf.for) is correct and per-iter compute is bit-equivalent to the 256-kernel envelope. Wall clock 236 ms for 100 seeds x 2 dispatches = ~1.18 ms/dispatch debug build amortized; runtime overhead per dispatch is comparable to the 256 kernel despite 32x more compute, which is the amortization win we wanted. Stage 1.4 verifier (#35) DONE; #34 production-shape work continues (#40 added: scale N_ITERS toward 27B Gemma full layer + multi-core fan-out so per-layer compute meets the ~600us BW-bound budget rather than ~33ms single-core).
