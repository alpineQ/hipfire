#!/usr/bin/env bash
# asym3_dequant_layer (N_ITERS=32 MVP) — hand-rolled native build.
#
# Same toolchain as asym3_dequant_256: clang++ Peano + native aiecc.
# No Python in the pipeline. Compiles the per-(head, position)
# C++ kernel and lowers the per-layer batched aie.mlir to PDI.
...
"$AIECC_BIN" \
    ...
    --peano="$PEANO" \
    --xclbin-name=asym3_dequant_layer.xclbin \
...
ls -la main.pdi insts.bin asym3_dequant_layer.xclbin 2>/dev/null || true
