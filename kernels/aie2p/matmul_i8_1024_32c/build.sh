#!/usr/bin/env bash
# matmul_i8_1024_32c - hand-rolled native build for 8c x 4-row = 32-tile
# i8 GEMM at M=K=N=1024 with tile (m, k, n) = (64, 64, 32) and
# COLUMN-MAJOR B layout.
#
# MLIR was generated once via mlir-aie's whole_array_placed.py with
# --dev=npu2 -M 1024 -K 1024 -N 1024 -m 64 -k 64 -n 32 --n-aie-cols 8
# --b-col-maj 1 --dtype_in i8 --dtype_out i32 - see aie.mlir in this dir.
#
# Why b-col-maj: row-major B at M=K=N=1024 with our default tile sizes
...
"$AIECC_BIN" \
    ...
    --peano="$PEANO" \
    --xclbin-name=matmul_i8_1024_32c.xclbin \
...
ls -la main.pdi insts.bin matmul_i8_1024_32c.xclbin 2>/dev/null || true
