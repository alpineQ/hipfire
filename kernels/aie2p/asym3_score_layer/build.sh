#!/usr/bin/env bash
# asym3_score_layer - hand-rolled native build for stage 2.6 MVP.
...
echo "[1/2] clang++ --target=aie2p-none-unknown-elf -std=c++20 -c asym3_score_kernel.cc"
"$PEANO/bin/clang++" \
    -O2 \
    -std=c++20 \
    --target=aie2p-none-unknown-elf \
    -fno-builtin-sqrtf -fno-builtin-sqrt \
    -Wno-parentheses -Wno-attributes -Wno-macro-redefined \
    -DNDEBUG \
    -I "$MLIR_AIE_INC" \
    -c asym3_score_kernel.cc \
    -o build/asym3_score_kernel.o
...
"$AIECC_BIN" \
    ...
    --peano="$PEANO" \
    --xclbin-name=asym3_score_layer.xclbin \
...
ls -la main.pdi insts.bin asym3_score_layer.xclbin 2>/dev/null || true
