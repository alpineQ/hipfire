//! `hipx-matmul-bf16-1024` — 1024×1024×1024 BF16 matmul. The natural-
//! precision LLM-applicable shape with enough compute to amortize
//! dispatch overhead.
...
const A_BYTES: usize = MATMUL_BF16_1024_4C_M * MATMUL_BF16_1024_4C_K * 2;
const B_BYTES: usize = MATMUL_BF16_1024_4C_K * MATMUL_BF16_1024_4C_N * 2;
const C_BYTES: usize = MATMUL_BF16_1024_4C_M * MATMUL_BF16_1024_4C_N * 4;
...
fn main() -> ExitCode {
    ...
    println!("[mmbf1k] device: AIE {}.{}, fw {}.{}.{} build {}",
    ...
    b.num_columns = MATMUL_BF16_1024_4C_COLUMNS;
    b.max_opc = MATMUL_BF16_1024_4C_OPS_PER_CYCLE;
    let ctx = match hipx.create_hwctx(&b) {
        Ok(c) => c,
        Err(e) => { eprintln!("create_hwctx: {e}"); return ExitCode::FAILURE; }
    };
    println!("[mmbf1k] hwctx={} cols={} M=K=N={} (4 cores, BF16)",
             ctx.handle, MATMUL_BF16_1024_4C_COLUMNS, MATMUL_BF16_1024_4C_M);

    let pdi_bo = hipx.alloc_dev(MATMUL_BF16_1024_4C_PDI.len()).expect("pdi alloc");
    unsafe {
    ...
    let instr_bo = hipx.alloc_dev(MATMUL_BF16_1024_4C_INSTS.len()).expect("instr alloc");
    unsafe {
    ...
    let mut a_host = vec![0f32; MATMUL_BF16_1024_4C_M * MATMUL_BF16_1024_4C_K];
    for r in 0..MATMUL_BF16_1024_4C_M {
        for kk in 0..MATMUL_BF16_1024_4C_K {
            a_host[r * MATMUL_BF16_1024_4C_K + kk] = ((r + kk) % 8) as f32 * 0.0625;
        }
    }
    let mut b_host = vec![0f32; MATMUL_BF16_1024_4C_K * MATMUL_BF16_1024_4C_N];
    for kk in 0..MATMUL_BF16_1024_4C_K {
        for c in 0..MATMUL_BF16_1024_4C_N {
            b_host[kk * MATMUL_BF16_1024_4C_N + c] = ((kk + c) % 8) as f32 * 0.0625;
        }
    }
    ...
    println!("[mmbf1k] BOs: A={A_BYTES} B B={B_BYTES} B C={C_BYTES} B");
    ...
    for r in 0..n_check {
        for c in 0..n_check {
            ...
            for kk in 0..MATMUL_BF16_1024_4C_K {
                want += a_host[r * MATMUL_BF16_1024_4C_K + kk] *
                        b_host[kk * MATMUL_BF16_1024_4C_N + c];
            }
            let off = (r * MATMUL_BF16_1024_4C_N + c) * 4;
    ...
    if errors == 0 {
        let macs = 2.0 * MATMUL_BF16_1024_4C_M as f64 * MATMUL_BF16_1024_4C_K as f64 *
                   MATMUL_BF16_1024_4C_N as f64;
        let tops = macs / (single_us as f64 / 1e6) / 1e12;
        println!("[mmbf1k] CORRECTNESS PASS — first {n_check}×{n_check} block within 5% rel-err (max abs err={max_err})");
        println!("[mmbf1k] single dispatch: {single_us} us → {tops:.2} TOp/s");
    } else {
        println!("[mmbf1k] CORRECTNESS FAIL: {errors}/{} mismatches >5% rel-err; first {first_bad:?}; max abs err={max_err}",
                 n_check * n_check);
        return ExitCode::FAILURE;
    }
    ...
    for _ in 0..n_iter {
        ...
        timeline_wait(hipx.device.fd, ctx.syncobj_handle, seq,
                      Duration::from_secs(20)).expect("wait");
    ...
    println!("[mmbf1k] perf: mean={mean_us}us max={max_us}us → {tops:.2} TOp/s ({n_iter} iters, {} GMACs)",
             (macs / 2e9) as u64);
