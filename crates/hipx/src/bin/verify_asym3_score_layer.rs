...
impl XorShift64 {
    fn new(seed: u64) -> Self { Self(if seed == 0 { 0xdeadbeefcafebabe } else { seed }) }
...
fn ref_match(packed: &[u8], cnorm: f32,
             c_mag: &[f32], c_abs: &[f32],
             cos_a: &[f32], sin_a: &[f32],
             cos_theta: &[f32], sin_theta: &[f32]) -> f32 {
    let mut s_trig = 0.0f32; let mut s_norm = 0.0f32;
    for tid in 0..32usize {
        let base = tid * 3;
        let bits = (packed[base] as u32) | ((packed[base+1] as u32) << 8)
                 | ((packed[base+2] as u32) << 16);
        let mut v = [0.0f32; 8];
        for i in 0..8 { v[i] = cnorm * TURBO_C3_256[((bits >> (3*i)) & 7) as usize]; }
        let b0 = tid * 4;
        for j in 0..4 {
            let f = b0 + j;
            let a_gv = v[j*2]; let b_gv = v[j*2+1];
...
#[allow(clippy::too_many_arguments)]
fn ref_igpu(packed: &[u8], cnorm: f32,
            c_re: &[f32], c_im: &[f32], c_abs: &[f32],
            omega: &[f32], p_q: f32,
            cos_theta: &[f32], sin_theta: &[f32]) -> f32 {
    let mut s_trig = 0.0f32; let mut s_norm = 0.0f32;
    for tid in 0..32usize {
        ...
        for j in 0..4 {
            ...
            let r = if c_abs[f] > 1e-20 { (cm / c_abs[f]).min(1.0) } else { 0.0 };
            s_norm += (1.0 - r) * c_abs[f] * k_mag;
        }
    }
    s_trig + s_norm
...
fn main() -> ExitCode {
    let n_seeds: usize = std::env::args().nth(1).and_then(|s| s.parse().ok()).unwrap_or(DEFAULT_SEEDS);
    let rope_theta: f32 = 10_000_000.0;
    let n_rot = HEAD_DIM;

    let pdi = load_pdi();
    let insts = load_insts();
    println!("[verify-score-layer] PDI {} bytes; insts {} bytes; N_ITERS={N_ITERS}; {n_seeds} seeds",
             pdi.len(), insts.len());

    let hipx_dev = match Hipx::open() {
        Ok(h) => h, Err(e) => { eprintln!("Hipx::open: {e}"); return ExitCode::FAILURE; }
    };
    let mut b = HwctxBuilder::default(); b.num_columns = 8; b.max_opc = 2048;
    let ctx = match hipx_dev.create_hwctx(&b) {
        Ok(c) => c, Err(e) => { eprintln!("create_hwctx: {e}"); return ExitCode::FAILURE; }
    };
    ...
    let _ = input_bo.map().expect("input map prime"); let _ = input_bo.sync(SYNC_TO_DEVICE);
    let _ = score_bo.map().expect("score map prime"); let _ = score_bo.sync(SYNC_TO_DEVICE);
    ...
    for seed in 1..=n_seeds as u64 {
        ...
        let mut cos_t = [0.0f32; N_BANDS]; let mut sin_t = [0.0f32; N_BANDS];
        for f in 0..N_BANDS {
            let theta = (rng.unit() - 0.5) * std::f32::consts::PI;
            cos_t[f] = theta.cos(); sin_t[f] = theta.sin();
        }
        let mut c_re = [0.0f32; N_BANDS]; let mut c_im = [0.0f32; N_BANDS];
        let mut c_mag = [0.0f32; N_BANDS]; let mut c_abs = [0.0f32; N_BANDS];
        let mut c_phase = [0.0f32; N_BANDS];
        for f in 0..N_BANDS {
            let r1 = (rng.unit() - 0.5) * 0.6; let r2 = (rng.unit() - 0.5) * 0.6;
            c_re[f] = r1; c_im[f] = r2;
        ...
        let mut omega = [0.0f32; N_BANDS]; let mut cos_a = [0.0f32; N_BANDS]; let mut sin_a = [0.0f32; N_BANDS];
        for f in 0..N_BANDS {
            omega[f] = if f < n_rot/2 { (-2.0 * (f as f32) / (n_rot as f32) * rope_theta.ln()).exp() } else { 0.0 };
            let angle = omega[f] * p_q + c_phase[f];
            cos_a[f] = angle.cos(); sin_a[f] = angle.sin();
        }
        ...
        for _it in 0..N_ITERS {
            ...
            for b in p.iter_mut() { *b = rng.next_byte(); }
...

        {
            ...
            for it in 0..N_ITERS {
                ...
                buf[base + OFF_PACKED..base + OFF_PACKED + PACKED_BYTES].copy_from_slice(&packed_per_iter[it]);
                buf[base + OFF_CNORM..base + OFF_CNORM + 4].copy_from_slice(&cnorm_per_iter[it].to_le_bytes());
                for (i, &v) in c_mag.iter().enumerate() {
                    buf[base + OFF_C_MAG + i*4..base + OFF_C_MAG + i*4 + 4].copy_from_slice(&v.to_le_bytes());
                }
                for (i, &v) in c_abs.iter().enumerate() {
                    buf[base + OFF_C_ABS + i*4..base + OFF_C_ABS + i*4 + 4].copy_from_slice(&v.to_le_bytes());
                }
                for (i, &v) in cos_a.iter().enumerate() {
                    buf[base + OFF_COS_A + i*4..base + OFF_COS_A + i*4 + 4].copy_from_slice(&v.to_le_bytes());
                }
                for (i, &v) in sin_a.iter().enumerate() {
                    buf[base + OFF_SIN_A + i*4..base + OFF_SIN_A + i*4 + 4].copy_from_slice(&v.to_le_bytes());
                }
                for (i, &v) in cos_t.iter().enumerate() {
                    buf[base + OFF_COS_T + i*4..base + OFF_COS_T + i*4 + 4].copy_from_slice(&v.to_le_bytes());
                }
                for (i, &v) in sin_t.iter().enumerate() {
                    buf[base + OFF_SIN_T + i*4..base + OFF_SIN_T + i*4 + 4].copy_from_slice(&v.to_le_bytes());
                }
            }
        }
        ...
        for run_idx in 0..2 {
            { let cbuf = cmd_bo.map().expect("cmd map"); hipx::ert::reset_state(&mut cbuf[..4]); }
            let _ = cmd_bo.sync(SYNC_TO_DEVICE);
            { let buf = score_bo.map().expect("score map");
              for b in buf[..SCORE_BYTES].iter_mut() { *b = 0xCC; } }
            let _ = score_bo.sync(SYNC_TO_DEVICE);
            let seq = match submit_exec_cmd(hipx_dev.device.fd, &ctx, &[&cmd_bo],
                                            &[&instr_bo, &input_bo, &score_bo, &bo3, &bo4]) {
                Ok(s) => s, Err(e) => { eprintln!("submit FAIL seed {seed}: {e}"); return ExitCode::FAILURE; }
            };
        ...
        for it in 0..N_ITERS {
            let bits = runs[0][it];
            if bits != runs[1][it] { nondet += 1;
                if first_fail.is_none() { first_fail = Some(format!("seed {seed} iter {it} non-deterministic")); }
                continue;
            }
            let npu = f32::from_bits(bits);
            if !npu.is_finite() || bits == SENTINEL {
                bad += 1;
                if first_fail.is_none() { first_fail = Some(format!("seed {seed} iter {it} bad npu=0x{bits:08x}")); }
                continue;
            }
            let m = ref_match(&packed_per_iter[it], cnorm_per_iter[it], &c_mag, &c_abs,
                              &cos_a, &sin_a, &cos_t, &sin_t);
            let g = ref_igpu(&packed_per_iter[it], cnorm_per_iter[it], &c_re, &c_im, &c_abs,
            ...
            if rel_m > max_rel_match { max_rel_match = rel_m; }
            if rel_g > max_rel_igpu  { max_rel_igpu  = rel_g; }
    ...
    println!();
    println!("=== verify_asym3_score_layer ({n_seeds} seeds x {N_ITERS} iters = {} compared) ===", n_compared);
    println!("  determinism:    {} ({} non-det iters)", if nondet == 0 { "PASS" } else { "FAIL" }, nondet);
    println!("  bad outputs:    {}", bad);
    println!("  max rel match:  {max_rel_match:.4e}  (bound 1e-2)");
    println!("  max rel iGPU:   {max_rel_igpu:.4e}  (bound 5e-2)");
    println!("  wall clock:     {:.3}s for {dispatches} dispatches = {per_dispatch:.3} ms/dispatch", elapsed.as_secs_f64());
