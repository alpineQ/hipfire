...
use hipx::kernels::{
    matmul_i8_2048_32c_args as args, MATMUL_I8_2048_32C_COLUMNS, MATMUL_I8_2048_32C_INSTS,
    MATMUL_I8_2048_32C_K, MATMUL_I8_2048_32C_M, MATMUL_I8_2048_32C_N,
    MATMUL_I8_2048_32C_OPS_PER_CYCLE, MATMUL_I8_2048_32C_PDI,
};
use hipx::Hipx;

const A_BYTES: usize = MATMUL_I8_2048_32C_M * MATMUL_I8_2048_32C_K;
const B_BYTES: usize = MATMUL_I8_2048_32C_K * MATMUL_I8_2048_32C_N;
const C_BYTES: usize = MATMUL_I8_2048_32C_M * MATMUL_I8_2048_32C_N * 4;
...
fn main() -> ExitCode {
    ...
    println!(
        "[mm8_2048_32c] device: AIE {}.{}, fw {}.{}.{} build {}",
    ...
    b.num_columns = MATMUL_I8_2048_32C_COLUMNS;
    b.max_opc = MATMUL_I8_2048_32C_OPS_PER_CYCLE;
    ...
    println!(
        "[mm8_2048_32c] hwctx={} cols={} M=K=N={} (32 cores, INT8, B col-major)",
        ctx.handle, MATMUL_I8_2048_32C_COLUMNS, MATMUL_I8_2048_32C_M
    );

    let pdi_bo = hipx
        .alloc_dev(MATMUL_I8_2048_32C_PDI.len())
        .expect("pdi alloc");
    unsafe {
        let buf = hipx.dev_slice(&pdi_bo).expect("pdi slice");
        buf[..MATMUL_I8_2048_32C_PDI.len()].copy_from_slice(MATMUL_I8_2048_32C_PDI);
    }
    let _ = pdi_bo.sync(SYNC_TO_DEVICE);
    let _cu = config_cus(hipx.device.fd, &ctx, vec![pdi_bo], &[0u8]).expect("config_cus");

    let instr_bo = hipx
        .alloc_dev(MATMUL_I8_2048_32C_INSTS.len())
        .expect("instr alloc");
    unsafe {
        let buf = hipx.dev_slice(&instr_bo).expect("instr slice");
        buf[..MATMUL_I8_2048_32C_INSTS.len()].copy_from_slice(MATMUL_I8_2048_32C_INSTS);
    }
    let _ = instr_bo.sync(SYNC_TO_DEVICE);
    let ninstr_dwords = (MATMUL_I8_2048_32C_INSTS.len() / 4) as u32;
    ...
    let mut a_host = vec![0i8; MATMUL_I8_2048_32C_M * MATMUL_I8_2048_32C_K];
    for r in 0..MATMUL_I8_2048_32C_M {
        for kk in 0..MATMUL_I8_2048_32C_K {
            let idx = (r * MATMUL_I8_2048_32C_K + kk) as u64;
            a_host[r * MATMUL_I8_2048_32C_K + kk] = pseudo_i8(idx);
        }
    }
    // B is column-major: b_host_cm[c * K + kk] = B[kk, c]
    let b_offset = (MATMUL_I8_2048_32C_M * MATMUL_I8_2048_32C_K) as u64;
    let mut b_host_cm = vec![0i8; MATMUL_I8_2048_32C_K * MATMUL_I8_2048_32C_N];
    for kk in 0..MATMUL_I8_2048_32C_K {
        for c in 0..MATMUL_I8_2048_32C_N {
            let logical_idx = b_offset + (kk * MATMUL_I8_2048_32C_N + c) as u64;
            let v = pseudo_i8(logical_idx);
            b_host_cm[c * MATMUL_I8_2048_32C_K + kk] = v;
        }
    }
    ...
    println!(
        "[mm8_2048_32c] BOs: A={A_BYTES} ({}KiB) B={B_BYTES} ({}KiB) C={C_BYTES} ({}KiB)",
        A_BYTES / 1024,
        B_BYTES / 1024,
        C_BYTES / 1024
    );
    ...
    let mut c_ref = vec![0i32; MATMUL_I8_2048_32C_M * MATMUL_I8_2048_32C_N];
    for r in 0..MATMUL_I8_2048_32C_M {
        for kk in 0..MATMUL_I8_2048_32C_K {
            let av = a_host[r * MATMUL_I8_2048_32C_K + kk] as i32;
            if av == 0 {
                continue;
            }
            let row_base = r * MATMUL_I8_2048_32C_N;
            for c in 0..MATMUL_I8_2048_32C_N {
                let bv = b_host_cm[c * MATMUL_I8_2048_32C_K + kk] as i32;
    ...
    let total_elems = MATMUL_I8_2048_32C_M * MATMUL_I8_2048_32C_N;
    if errors == 0 {
        let macs = 2.0 * MATMUL_I8_2048_32C_M as f64
            * MATMUL_I8_2048_32C_K as f64
            * MATMUL_I8_2048_32C_N as f64;
        let gops = macs / (single_us as f64 / 1e6) / 1e9;
        println!(
            "[mm8_2048_32c] CORRECTNESS PASS - all {total_elems} output elements match"
        );
        println!(
            "[mm8_2048_32c] single dispatch: {single_us} us -> {gops:.2} GOp/s on {} GMACs",
            macs as u64 / 1_000_000_000
        );
    } else {
        println!(
            "[mm8_2048_32c] CORRECTNESS FAIL: {errors}/{total_elems} mismatches; first {first_bad:?}"
        );
        return ExitCode::FAILURE;
    }
    ...
    println!(
        "[mm8_2048_32c] perf: mean={mean_us}us max={max_us}us -> {tops:.2} TOp/s ({n_iter} iters, {} GMACs)",
        (macs / 2.0 / 1e9) as u64
    );
