//! `hipx-matmul-i8-2048` — 2048×2048×2048 INT8 matmul on AIE-2P.
//! 8× the MAC volume of the 1024^3 variant; closest we get to the
//! saturating regime where the AIE compute fraction approaches 1.
...
use hipx::kernels::{
    matmul_i8_2048_4c_args as args, MATMUL_I8_2048_4C_COLUMNS, MATMUL_I8_2048_4C_INSTS,
    MATMUL_I8_2048_4C_K, MATMUL_I8_2048_4C_M, MATMUL_I8_2048_4C_N,
    MATMUL_I8_2048_4C_OPS_PER_CYCLE, MATMUL_I8_2048_4C_PDI,
};
use hipx::Hipx;

const A_BYTES: usize = MATMUL_I8_2048_4C_M * MATMUL_I8_2048_4C_K;
const B_BYTES: usize = MATMUL_I8_2048_4C_K * MATMUL_I8_2048_4C_N;
const C_BYTES: usize = MATMUL_I8_2048_4C_M * MATMUL_I8_2048_4C_N * 4;
...
fn main() -> ExitCode {
    ...
    println!("[mm8_2048] device: AIE {}.{}, fw {}.{}.{} build {}",
    ...
    b.num_columns = MATMUL_I8_2048_4C_COLUMNS;
    b.max_opc = MATMUL_I8_2048_4C_OPS_PER_CYCLE;
    let ctx = match hipx.create_hwctx(&b) {
        Ok(c) => c,
        Err(e) => { eprintln!("create_hwctx: {e}"); return ExitCode::FAILURE; }
    };
    println!("[mm8_2048] hwctx={} cols={} M=K=N={} (4 cores, INT8)",
             ctx.handle, MATMUL_I8_2048_4C_COLUMNS, MATMUL_I8_2048_4C_M);

    let pdi_bo = hipx.alloc_dev(MATMUL_I8_2048_4C_PDI.len()).expect("pdi alloc");
    unsafe {
        let buf = hipx.dev_slice(&pdi_bo).expect("pdi slice");
        buf[..MATMUL_I8_2048_4C_PDI.len()].copy_from_slice(MATMUL_I8_2048_4C_PDI);
    }
    let _ = pdi_bo.sync(SYNC_TO_DEVICE);
    let _cu = config_cus(hipx.device.fd, &ctx, vec![pdi_bo], &[0u8])
        .expect("config_cus");

    let instr_bo = hipx.alloc_dev(MATMUL_I8_2048_4C_INSTS.len()).expect("instr alloc");
    unsafe {
        let buf = hipx.dev_slice(&instr_bo).expect("instr slice");
        buf[..MATMUL_I8_2048_4C_INSTS.len()].copy_from_slice(MATMUL_I8_2048_4C_INSTS);
    }
    let _ = instr_bo.sync(SYNC_TO_DEVICE);
    let ninstr_dwords = (MATMUL_I8_2048_4C_INSTS.len() / 4) as u32;

    // Use very small values so 2048 accum stays in i32 cleanly.
    let mut a_host = vec![0i8; MATMUL_I8_2048_4C_M * MATMUL_I8_2048_4C_K];
    for r in 0..MATMUL_I8_2048_4C_M {
        for kk in 0..MATMUL_I8_2048_4C_K {
            a_host[r * MATMUL_I8_2048_4C_K + kk] = ((r + kk) as i8) & 0x1;
        }
    }
    let mut b_host = vec![0i8; MATMUL_I8_2048_4C_K * MATMUL_I8_2048_4C_N];
    for kk in 0..MATMUL_I8_2048_4C_K {
        for c in 0..MATMUL_I8_2048_4C_N {
            b_host[kk * MATMUL_I8_2048_4C_N + c] = ((kk + c) as i8) & 0x1;
        }
    }
    ...
    println!("[mm8_2048] BOs: A={A_BYTES} B B={B_BYTES} B C={C_BYTES} B");
    ...
    for r in 0..n_check {
        for c in 0..n_check {
            let mut want: i32 = 0;
            for kk in 0..MATMUL_I8_2048_4C_K {
                want += a_host[r * MATMUL_I8_2048_4C_K + kk] as i32 *
                        b_host[kk * MATMUL_I8_2048_4C_N + c] as i32;
            }
            let off = (r * MATMUL_I8_2048_4C_N + c) * 4;
    ...
    if errors == 0 {
        let macs = 2.0 * MATMUL_I8_2048_4C_M as f64 * MATMUL_I8_2048_4C_K as f64 *
                   MATMUL_I8_2048_4C_N as f64;
        let tops = macs / (single_us as f64 / 1e6) / 1e12;
        println!("[mm8_2048] CORRECTNESS PASS — first {n_check}×{n_check} block matches");
        println!("[mm8_2048] single dispatch: {single_us} us → {tops:.2} TOp/s on {} GMACs",
                 macs as u64 / 1_000_000_000);
    } else {
        println!("[mm8_2048] CORRECTNESS FAIL: {errors}/{} mismatches; first {first_bad:?}",
                 n_check * n_check);
        return ExitCode::FAILURE;
    }
    ...
    for _ in 0..n_iter {
        ...
        timeline_wait(hipx.device.fd, ctx.syncobj_handle, seq,
                      Duration::from_secs(30)).expect("wait");
    ...
    println!("[mm8_2048] perf: mean={mean_us}us max={max_us}us → {tops:.2} TOp/s ({n_iter} iters, {} GMACs)",
             (macs / 2.0 / 1e9) as u64);
