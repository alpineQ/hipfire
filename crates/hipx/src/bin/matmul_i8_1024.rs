//! `hipx-matmul-i8-1024` — 1024×1024×1024 INT8 matmul on AIE-2P.
//! 8× the MAC volume of the 512^3 variant; the dispatch overhead
//! amortizes proportionally so this is where AIE compute fraction
//! becomes >50%.
...
use hipx::kernels::{
    matmul_i8_1024_4c_args as args, MATMUL_I8_1024_4C_COLUMNS, MATMUL_I8_1024_4C_INSTS,
    MATMUL_I8_1024_4C_K, MATMUL_I8_1024_4C_M, MATMUL_I8_1024_4C_N,
    MATMUL_I8_1024_4C_OPS_PER_CYCLE, MATMUL_I8_1024_4C_PDI,
};
use hipx::Hipx;

const A_BYTES: usize = MATMUL_I8_1024_4C_M * MATMUL_I8_1024_4C_K;
const B_BYTES: usize = MATMUL_I8_1024_4C_K * MATMUL_I8_1024_4C_N;
const C_BYTES: usize = MATMUL_I8_1024_4C_M * MATMUL_I8_1024_4C_N * 4;
...
fn main() -> ExitCode {
    ...
    println!("[mm8_1024] device: AIE {}.{}, fw {}.{}.{} build {}",
    ...
    b.num_columns = MATMUL_I8_1024_4C_COLUMNS;
    b.max_opc = MATMUL_I8_1024_4C_OPS_PER_CYCLE;
    let ctx = match hipx.create_hwctx(&b) {
        Ok(c) => c,
        Err(e) => { eprintln!("create_hwctx: {e}"); return ExitCode::FAILURE; }
    };
    println!("[mm8_1024] hwctx={} cols={} M=K=N={} (4 cores, INT8)",
             ctx.handle, MATMUL_I8_1024_4C_COLUMNS, MATMUL_I8_1024_4C_M);

    let pdi_bo = hipx.alloc_dev(MATMUL_I8_1024_4C_PDI.len()).expect("pdi alloc");
    unsafe {
    ...
    let instr_bo = hipx.alloc_dev(MATMUL_I8_1024_4C_INSTS.len()).expect("instr alloc");
    unsafe {
    ...
    // Use small values so 1024 accum stays cleanly representable.
    let mut a_host = vec![0i8; MATMUL_I8_1024_4C_M * MATMUL_I8_1024_4C_K];
    for r in 0..MATMUL_I8_1024_4C_M {
        for kk in 0..MATMUL_I8_1024_4C_K {
            a_host[r * MATMUL_I8_1024_4C_K + kk] = ((r + kk) as i8) & 0x3;
        }
    }
    let mut b_host = vec![0i8; MATMUL_I8_1024_4C_K * MATMUL_I8_1024_4C_N];
    for kk in 0..MATMUL_I8_1024_4C_K {
        for c in 0..MATMUL_I8_1024_4C_N {
            b_host[kk * MATMUL_I8_1024_4C_N + c] = ((kk + c) as i8) & 0x3;
        }
    }
    ...
    println!("[mm8_1024] BOs: A={A_BYTES} ({}KiB) B={B_BYTES} ({}KiB) C={C_BYTES} ({}KiB)",
    ...
    for r in 0..n_check {
        for c in 0..n_check {
            let mut want: i32 = 0;
            for kk in 0..MATMUL_I8_1024_4C_K {
                want += a_host[r * MATMUL_I8_1024_4C_K + kk] as i32 *
                        b_host[kk * MATMUL_I8_1024_4C_N + c] as i32;
            }
            let off = (r * MATMUL_I8_1024_4C_N + c) * 4;
    ...
    if errors == 0 {
        let macs = 2.0 * MATMUL_I8_1024_4C_M as f64 * MATMUL_I8_1024_4C_K as f64 *
                   MATMUL_I8_1024_4C_N as f64;
        let gops = macs / (single_us as f64 / 1e6) / 1e9;
        println!("[mm8_1024] CORRECTNESS PASS — first {n_check}×{n_check} block matches");
        println!("[mm8_1024] single dispatch: {single_us} us → {gops:.2} GOp/s on {} GMACs",
                 macs as u64 / 1_000_000_000);
    } else {
        println!("[mm8_1024] CORRECTNESS FAIL: {errors}/{} mismatches; first {first_bad:?}",
                 n_check * n_check);
        return ExitCode::FAILURE;
    }
    ...
    println!("[mm8_1024] perf: mean={mean_us}us max={max_us}us → {tops:.2} TOp/s ({n_iter} iters, {} GMACs)",
             (macs / 2.0 / 1e9) as u64);
